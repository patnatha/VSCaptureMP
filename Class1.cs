...
namespace VSCaptureMP
{
    public class MPudpclient : UdpClient
    {
        ...
        public string m_RCUrl;
        public string m_RCToken;
        public string m_RCWhichOR;
        public DateTime m_RC_DateTime = DateTime.Now;
        public Dictionary<string, string> m_RC_Data = new Dictionary<string, string>();
        ...
        public MPudpclient()
        {
            ...
            //Set default time in initialization of the data structure
            m_RC_Data.Add("datetime", "null");
        }
        ...
        public void PollPacketDecoder(byte[] packetbuffer, int headersize)
        {
            ...
            if (scpollobjectscount > 0)
            {
                ...
                if (m_dataexportset == 1)
                {
                    ExportDataToCSV();
                    ExportWaveToCSV();
                }
                else if (m_dataexportset == 2)
                {
        ...
        public void ExportNumValListToJSON()
        {
            ...
            //If been 10 seconds since the last record and data been added, then post them
            if ((cur_Datetime - m_RC_DateTime).TotalSeconds >= 10 && m_RC_Data.Count > 1)
            {
                //Build the list of curent data to upload
                List<Dictionary<string, string>> json_data_field = new List<Dictionary<string, string>>();
                json_data_field.Add(m_RC_Data);

                Dictionary<string, string> postHeader = new Dictionary<string, string>();
                postHeader.Add("token", m_RCToken);
                postHeader.Add("content", "record");
                postHeader.Add("action", "import");
                postHeader.Add("format", "json");
                postHeader.Add("type", "flat");
                postHeader.Add("overwriteBehavior", "normal");
                postHeader.Add("forceAutoNumber", "true");
                postHeader.Add("returnContent", "count");
                postHeader.Add("returnFormat", "json");
                postHeader.Add("data", "");
                postHeader["data"] = JsonSerializer.Serialize(json_data_field, new JsonSerializerOptions { IncludeFields = true });

                try
                {
                    Console.WriteLine("POSTING: " + postHeader["data"]);
                    //Console.WriteLine("Length Wave: " + m_WaveValResultList.Count.ToString());
                    Task.Run(() => PostJSONDataToServer(postHeader));
                }
                ...
                //Clear the structure and reset the null timestamp
                m_RC_Data.Clear();
                m_RC_Data["datetime"] = "null";
            }

            foreach (NumericValResult NumValResult in m_NumericValList)
            {
                if (NumValResult.Value != "-")
                {
                    Console.WriteLine(NumValResult.PhysioID + ":" + NumValResult.Value);

                    if (NumValResult.PhysioID == "NOM_TEMP")
                    {
                    ...
                    else if (NumValResult.PhysioID == "NOM_AWAY_RESP_RATE")
                    {
                        m_RC_Data.Add("resp_rate", NumValResult.Value);
                    }
            ...
            if (m_RC_Data.Count() > 1 && m_RC_Data["datetime"] == "null")
            {
                //Add the current time as the time stamp
                m_RC_Data["datetime"] = cur_Datetime.ToString("yyyy-MM-dd HH:mm:ss");
                m_RC_DateTime = cur_Datetime;

                //Add which OR the data is coming from
                m_RC_Data.Add("or", m_RCWhichOR);
        ...
        public async Task PostJSONDataToServer(Dictionary<string, string> postData)
        {
            using (HttpClient client = new HttpClient())
            {
                //Set the security protocols allowed
                ServicePointManager.SecurityProtocol |= SecurityProtocolType.Tls | SecurityProtocolType.Tls11 | SecurityProtocolType.Tls12;

                //Format the content
                var data = new FormUrlEncodedContent(postData);

                //Post the result and get the response
                var response = await client.PostAsync(m_RCUrl, data);
                string result = await response.Content.ReadAsStringAsync();

                if (response.StatusCode == HttpStatusCode.OK)
                {
                    Dictionary<String, String> countRes = JsonSerializer.Deserialize<Dictionary<string, string>>(result);
                    Console.WriteLine("RC 200" + result);
                }
                else
                {
                    //Throw error
                    Console.WriteLine("RC ERROR" + result);
                }
            }
        }
    }
